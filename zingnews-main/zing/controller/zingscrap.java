...
public class zingscrap {
        ...
        try {
            ...
            Elements zingTable = doc.select("div.article-list.listing-layout.responsive.infinite-load article");
    ...
    public static String list_read_zing(String zingUrl, ArrayList<FeedItem> news_detail) {
        try {
            Document doc = Jsoup.connect(zingUrl).timeout(6000).get();
            Elements zingTable = doc.select("div.article-list.listing-layout.responsive.unique article");
            for (Element article : zingTable.select("p.article-thumbnail")) {
                Elements src = article.select("a");
                String slug = src.attr("href");
                Document inside_out = Jsoup.connect(zingUrl.concat(slug)).timeout(4000).get();

                Elements zingDescription = inside_out.select("div.page-wrapper article div.the-article-body");

                Elements zingHeader = inside_out.select("div.page-wrapper article ");
                for (Element rows : zingDescription) {
